...
class TestCommandParsing:
    ...
    def test_handle_help(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        result = handle_command(
            "help", "help", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
        assert result is True
    ...
    def test_handle_quit_raises(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        with pytest.raises(SystemExit):
            handle_command(
                "quit", "quit", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
            )
    ...
    def test_handle_exit_alias(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        with pytest.raises(SystemExit):
            handle_command(
                "exit", "exit", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
            )
...
class TestFileCommands:
    def test_cmd_files(self, tmp_path, mock_console, mock_client, messages, ctx_files):
        ...
        handle_command(
            "files", "files", messages, mock_client, mock_console, str(tmp_path), ctx_files
        )

    def test_cmd_tree(self, tmp_path, mock_console, mock_client, messages, ctx_files):
        (tmp_path / "a.py").write_text("# a")
        handle_command(
            "tree", "tree --depth 1", messages, mock_client, mock_console, str(tmp_path), ctx_files
        )

    def test_cmd_add(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "add", "add myfile.py", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
...
class TestSessionCommands:
    def test_cmd_clear(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        ...
        handle_command(
            "clear", "clear", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
    ...
    def test_cmd_version(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "version", "version", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
    ...
    def test_cmd_status(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "status", "status", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )

    def test_cmd_compact_few_messages(
        self, mock_console, mock_client, messages, tmp_cwd, ctx_files
    ):
        messages.append({"role": "user", "content": "hi"})
        handle_command(
            "compact", "compact", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
        assert len(messages) == 1  # Not enough to compact

    def test_cmd_compact_many_messages(
        self, mock_console, mock_client, messages, tmp_cwd, ctx_files
    ):
        for i in range(10):
...
class TestProviderCommands:
    def test_cmd_model_show(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "model", "model", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
    ...
    def test_cmd_models(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "models", "models", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )

    def test_cmd_provider_show(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "provider", "provider", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )

    def test_cmd_providers(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        with patch(
            "termmind.commands.load_config", return_value={"provider": "ollama", "api_key": ""}
        ):
            handle_command(
                "providers",
                "providers",
...
class TestThemeCommands:
    def test_cmd_theme_show(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        with patch("termmind.commands.load_config", return_value={"theme": "dark"}):
            handle_command(
                "theme", "theme", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
            )

    def test_cmd_themes(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "themes", "themes", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
...
class TestGitCommands:
    def test_cmd_git_no_repo(self, tmp_path, mock_console, mock_client, messages, ctx_files):
        handle_command(
            "git", "git status", messages, mock_client, mock_console, str(tmp_path), ctx_files
        )
...
class TestSearchCommands:
    def test_cmd_search_no_args(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "search", "search", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )

    def test_cmd_search(self, tmp_path, mock_console, mock_client, messages, ctx_files):
        (tmp_path / "test.py").write_text("def hello(): pass")
        handle_command(
            "search", "search hello", messages, mock_client, mock_console, str(tmp_path), ctx_files
        )
    ...
    def test_cmd_grep(self, tmp_path, mock_console, mock_client, messages, ctx_files):
        ...
        handle_command(
            "grep", "grep import", messages, mock_client, mock_console, str(tmp_path), ctx_files
        )
...
class TestRunCommand:
    ...
    def test_run_command(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "run", "run echo hello", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
...
class TestUndoCommands:
    ...
    def test_undo_all_empty(self, mock_console, mock_client, messages, tmp_cwd, ctx_files):
        handle_command(
            "undo", "undo --all", messages, mock_client, mock_console, str(tmp_cwd), ctx_files
        )
